...
class FakeTextDataGenerator(object):
    ...
    def generate(
        ...
        # Horizontal text
        if orientation == 0:
            denominator = float(distorted_img.size[1])
            if denominator == 0:
                print("Warning: Image height is zero. Skipping...")
                return

            target_height = size - vertical_margin
            if target_height <= 0:
                print("Warning: Target height is non-positive. Skipping...")
                return

            new_width = int(distorted_img.size[0] * (float(target_height) / denominator))
            if new_width <= 0:
                print("Warning: Computed new_width is non-positive. Skipping...")
                return
            ...
            resized_mask = distorted_mask.resize(
                (new_width, target_height), Image.Resampling.NEAREST
            )
            background_width = width if width > 0 else new_width + horizontal_margin
            background_height = size
        ...
        # Vertical text
            ...
            denominator = float(distorted_img.size[0])
            if denominator == 0:
                print("Warning: Image width is zero. Skipping...")
                return

            target_width = size - horizontal_margin
            if target_width <= 0:
                print("Warning: Target width is non-positive. Skipping...")
                return

            new_height = int(distorted_img.size[1] * (float(target_width) / denominator))
            if new_height <= 0:
                print("Warning: Computed new_height is non-positive. Skipping...")
                return
            ...
            resized_mask = distorted_mask.resize(
                (target_width, new_height), Image.Resampling.NEAREST
            )
            background_width = size
            background_height = new_height + vertical_margin
